...
```
대충 요런 느낌으로 실행하시면 됩니다. 그리고 코드 돌릴때 구글 드라이브에 rre라는 폴더를 만들고 그 안에다가 코드 넣게 해야 돌아갈거에요. 그리고 이게 FRED API를 써서 그거 코렙에다가 등록 하고 쓰셔야 해요.
...
아 그리고 lightgbm으로 선택한 이유는 사실 제가 LSTM으로도 구현 해봤는데 자료를 좀더 찾아보다 보니까 주식ai에서는 LSTM보다는 lightgbm이 오히려 더 성능이 좋게 나온다는 의견이 많길레 lstm모델을 lightgbm으로 바꿨습니다. (근데 성능 차이 별로 없던데..... 그래도 lightgbm은 cpu로 학습이 가능하니까 가성비가 더 좋은 거 같습니다) 
